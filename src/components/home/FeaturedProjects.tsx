...
const projects = [
  ...
  },
  {
    ...
    category: 'Restaurant',
    date: '8/8/24',
    title: 'Essence Restaurant',
    services: 'Web design & Web development',
    image: '/restaurant-preview.png',
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {projects.map((project, index) => (
            <Link key={project.id} href={project.link} passHref className="block">
              <motion.div
                initial={{ opacity: 0, y: 50 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: index * 0.1 }}
                className="group relative bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 cursor-pointer h-full"
              >
                {/* Project Image */}
