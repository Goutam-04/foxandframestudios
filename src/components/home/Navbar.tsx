...
export default function Navbar() {
  ...
  return (
    <motion.nav
      initial={{ y: -100 }}
      animate={{ y: 0 }}
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-[#EEE9E3]/90 backdrop-blur-md shadow-sm py-4' : 'bg-transparent py-6'
        }`}
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        {/* Logo */}
        <Link href="/home" className="flex items-center gap-3">
          <img src="/logo.png" alt="Fox & Frame" className="h-12 w-auto object-contain" />
        </Link>
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <Link href="/contact" className="text-[#2A2A2A] hover:text-[#2A2A2A]/70 transition-colors font-medium">
            Contact
          </Link>
          <Link href="/contact">
